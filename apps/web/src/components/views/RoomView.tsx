      ...
      <div className="flex-1 overflow-auto p-4">
        <div className="max-w-2xl mx-auto space-y-6">
          ...
          <div className="flex gap-1 bg-slate-800/50 rounded-lg p-1">
            ...
            >
              <Inbox className="w-5 h-5 sm:w-4 sm:h-4" />
              Inbox
            </button>
            <button
          ...
          <Card variant="bordered" className="flex flex-col overflow-hidden">
            <div className="flex-1 min-h-0 overflow-y-auto flex flex-col space-y-4">
              {activeTab === 'outbox' && (
                <>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Send className="w-6 h-6 sm:w-5 sm:h-5 text-teal-400" />
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        className="flex items-center gap-1.5 text-xs font-medium text-teal-400 hover:text-teal-300 transition-colors px-2.5 py-1.5 sm:px-2 sm:py-1 rounded-md hover:bg-teal-400/10"
                        aria-label="Add file to share"
                        title="Add file to share"
                      >
                        <FileUp className="w-5 h-5 sm:w-3.5 sm:h-3.5" />
                      ...
                      {canUsePasteButton && (
                        <button
                          ...
                          disabled={isPasting}
                          className="flex items-center gap-1.5 text-xs font-medium text-teal-400 hover:text-teal-300 transition-colors px-2.5 py-1.5 sm:px-2 sm:py-1 rounded-md hover:bg-teal-400/10 disabled:opacity-50"
                          aria-label="Paste from clipboard"
                          title="Paste from clipboard"
                        >
                          <ClipboardPaste className="w-5 h-5 sm:w-3.5 sm:h-3.5" />
                          <span className="hidden sm:inline">{isPasting ? 'Pasting…' : 'Paste'}</span>
                        </button>
                      )}
                    </div>
                  </div>
                  {/* Paste accepts + type filter: hidden for now (pasteTypeFilter stays "all"); code kept for later */}
                  {false && (
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {PASTE_ACCEPT_TYPES.map((opt) => (
                        <button
                          ...
                          className={`px-2 py-0.5 text-xs rounded transition-colors ${pasteTypeFilter === opt.id
              ...
              <FileList direction={activeTab} onDownload={onRequestFile} onCopyTextFile={onCopyTextFile} hideHeader={activeTab === 'outbox'} hideFilters />
            </div>
          </Card>
